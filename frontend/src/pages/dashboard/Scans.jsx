...
import { ScanLine, Plus, Upload, Zap, Trash2, Eye, Loader2 } from "lucide-react";
...
export default function Scans() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault(); if (!selectedPatient) { setError("Select a patient"); return; }
  ...
  const handleAnalyze = async (scanId) => { setAnalyzing(scanId); try { await analyzeScan(scanId); toast.success("Analysis complete"); load(); } catch (err) { toast.error(err.message); } finally { setAnalyzing(null); } };
  const handleDelete = async (scanId) => { try { await deleteScan(scanId); toast.success("Scan deleted"); setConfirmDelete(null); load(); } catch (err) { toast.error(err.message); } };
    ...
    <div className="space-y-6">
            ...
            return (
              <div key={s.scan_id} className="px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-[var(--surface-card)] rounded-[16px] flex items-center justify-center text-[var(--mute)]"><ScanLine className="w-4 h-4" /></div>
                  <div>
                    <p className="text-sm font-semibold text-[var(--ink)]">{patient?.name || "Unknown"} — {s.scan_type?.replace("_", " ")}</p>
                    <p className="text-xs text-[var(--ash)]">{s.image_path ? "Image uploaded" : "No image"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {s.status === "uploaded" && s.image_path && (
                    <button onClick={() => handleAnalyze(s.scan_id)} disabled={analyzing === s.scan_id} className="px-3 py-1.5 text-xs font-bold text-[var(--on-primary)] bg-[var(--primary)] rounded-[9999px] hover:bg-[var(--primary-pressed)] disabled:opacity-50 flex items-center gap-1 transition-colors">
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="New X-Ray Scan">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <select value={scanType} onChange={(e) => setScanType(e.target.value)} className="w-full px-4 py-3 bg-[var(--surface-card)] border border-[var(--hairline)] rounded-[16px] text-sm focus:outline-none focus:border-[var(--ink)]">
            <option value="chest_xray">Chest X-Ray</option><option value="hand_xray">Hand X-Ray</option><option value="dental_xray">Dental</option><option value="other">Other</option>
          </select>
          <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-[var(--hairline)] rounded-[16px] p-8 text-center cursor-pointer hover:border-[var(--ash)] transition-colors">
            ...
            {selectedFile ? <span className="text-sm font-semibold text-[var(--ink)]">{selectedFile.name}</span> : <span className="text-sm text-[var(--ash)]">Click to upload X-ray image</span>}
          </div>
